...
create table if not exists public.profiles (
  ...
  preferred_work_types jsonb not null default '[]'::jsonb,
...
create table if not exists public.driver_logs (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  date date not null,
  work_type text not null default 'driver' check (work_type in ('driver', 'cleaner', 'restaurant_worker', 'server_waiter', 'warehouse', 'construction', 'landscaping', 'delivery_courier', 'other')),
  platform text not null check (platform in ('Uber', 'Lyft', 'DoorDash', 'Uber Eats', 'Amazon Flex', 'Spark', 'Instacart', 'OnTrac', 'House cleaning', 'Office cleaning', 'Airbnb cleaning', 'Hotel cleaning', 'Company', 'Private client', 'Restaurant', 'Bar', 'Cafe', 'Catering', 'Hotel', 'Warehouse', 'Amazon', 'FedEx', 'UPS', 'Walmart', 'Construction', 'Contractor', 'Day labor', 'Landscaping', 'Lawn care', 'Snow removal', 'Other')),
  start_time time,
  end_time time,
  miles_driven numeric(10, 2) not null default 0,
  gross_earnings numeric(12, 2) not null default 0,
  ...
  stops_completed numeric(10, 0) not null default 0,
